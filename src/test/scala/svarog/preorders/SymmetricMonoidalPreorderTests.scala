...
class SymmetricMonoidalPreorderTests extends Properties {
  ...
  def tests: List[Prop] = List.empty ++
    ...
    mkSymmetricMonoidalPreorderLaws(mkBooleanGen, "(Boolean,<=,||,false)")(SymmetricMonoidalPreorder.SMPBoolOrLe) ++
    mkSymmetricMonoidalPreorderLaws(mkBooleanGen, "(Double,<=,&&,true)")(SymmetricMonoidalPreorder.SMPBoolAndLe) ++
